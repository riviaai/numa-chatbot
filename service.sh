#!/bin/bash
# ============================================================
# Numa (Numerologie Chatbot) Service Manager
...
case "$1" in
  start)
    ...
    PID=$(lsof -ti tcp:$PORT 2>/dev/null)
    if [ -n "$PID" ]; then
      echo "Numa started successfully on port $PORT (PID: $PID)"
    else
  ...
  status)
    PID=$(lsof -ti tcp:$PORT 2>/dev/null)
    if [ -n "$PID" ]; then
      echo "Numa is RUNNING on port $PORT (PID: $PID)"
